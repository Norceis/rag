...
how_many_docs = 9500
doc_format = 'txt'
args_iterable = ((rfc_number, scrapping_logger, doc_format) for rfc_number in range(how_many_docs))

with concurrent.futures.ThreadPoolExecutor(max_workers=24) as executor:
    for _ in tqdm(executor.map(lambda args: download_rfc(*args), args_iterable), total=how_many_docs):
        pass
